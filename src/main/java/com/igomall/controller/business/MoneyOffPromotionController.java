...
import com.igomall.entity.MoneyOffAttribute;
...
public class MoneyOffPromotionController extends BaseController {
	...
	public void populateModel(Long promotionId, Long moneyOffAttributeId, @CurrentStore Store currentStore, ModelMap model) {
		...
		MoneyOffAttribute moneyOffAttribute = (MoneyOffAttribute) promotionDefaultAttributeService.find(moneyOffAttributeId);
	...
	public String add(String promotionPluginId, @CurrentStore Store currentStore, ModelMap model) {
		...
		model.addAttribute("discountTypes", MoneyOffAttribute.DiscountType.values());
	 ...
	 */
	@PostMapping("/save")
	public ResponseEntity<?> save(@ModelAttribute("promotionForm") Promotion promotionForm, @ModelAttribute("moneyOffAttributeForm") MoneyOffAttribute moneyOffAttributeForm, Long[] memberRankIds, @CurrentStore Store currentStore) {
	 ...
	 */
	@PostMapping("/update")
	public ResponseEntity<?> update(@ModelAttribute("promotionForm") Promotion promotionForm, @ModelAttribute(binding = false) Promotion promotion, @ModelAttribute("moneyOffAttributeForm") MoneyOffAttribute moneyOffAttributeForm,
			@ModelAttribute(binding = false) MoneyOffAttribute moneyOffAttribute, Long[] memberRankIds) {
		if (promotion == null) {
			return Results.UNPROCESSABLE_ENTITY;
		}
		promotionForm.setMemberRanks(new HashSet<>(memberRankService.findList(memberRankIds)));
