...
public class CouponServiceImpl extends BaseServiceImpl<Coupon> implements CouponService {
	...
	public List<Coupon> findList(Store store, Set<Coupon> matchs) {
		return couponDao.selectList(store, matchs);
	}

	@Override
	@Transactional(readOnly = true)
	public Page<Coupon> findPage(Boolean isEnabled, Boolean isExchange, Boolean hasExpired, Pageable pageable) {
		IPage<Coupon> iPage = getPluginsPage(pageable);
		iPage.setRecords(couponDao.findPage(iPage, getPageable(pageable), isEnabled, isExchange, hasExpired));
	...
	public Page<Coupon> findPage(Store store, Pageable pageable) {
		IPage<Coupon> iPage = getPluginsPage(pageable);
		iPage.setRecords(couponDao.findPageStore(iPage, getPageable(pageable), store));
		return super.findPage(iPage, pageable);
	}
