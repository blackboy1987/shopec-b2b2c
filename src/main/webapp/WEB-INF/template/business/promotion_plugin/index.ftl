...
<head>
	...
	<title>${message("business.promotionPlugin.index")} - Powered By SHOPEC</title>
								...
								[#if currentStore.type == "GENERAL" ]
									...
									[#if currentStore.isPromotionPluginActive(promotionPlugin.id)]
										<span class="pull-right">${message("StorePluginStatus.endDate")}:
											<span title="${currentStore.getPromotionPluginEndDate(promotionPlugin.id)?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${currentStore.getPromotionPluginEndDate(promotionPlugin.id)?string("yyyy-MM-dd HH:mm:ss")}</span>	
										</span>
									[#else]
