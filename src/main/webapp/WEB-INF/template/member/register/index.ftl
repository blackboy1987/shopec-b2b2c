...
<head>
	...
	<title>${message("member.register.title")} - Powered By SHOPEC</title>
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				// 用户注册成功
				$registerForm.on("success.shopec.ajaxSubmit", function() {
					$document.trigger("registered.shopec.user", [{
						type: "member",
				...
				// 验证码图片
				$registerForm.on("error.shopec.ajaxSubmit", function() {
					$captchaImage.captchaImage("refresh");
				});
				
				// 验证码图片
				$captchaImage.on("refreshed.shopec.captchaImage", function() {
					$captcha.val("");
				});
