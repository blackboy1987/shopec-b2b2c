...
<head>
	...
	[@seo type = "ARTICLE_LIST"]
		...
		[/#if]
		<title>[#if articleCategory.seoTitle?has_content]${articleCategory.seoTitle}[#else]${seo.resolveTitle()}[/#if][#if showPowered] - Powered By SHOPEC[/#if]</title>
	[/@seo]
									...
									[#list page.content as article]
										<li>
											...
											<p>
												...
												[/#if]
												<span class="small text-gray" title="${article.createdDate?string("yyyy-MM-dd HH:mm:ss")}">${article.createdDate?string("yyyy-MM-dd HH:mm:ss")}</span>
											</p>
										</li>
									[/#list]
								</ul>
