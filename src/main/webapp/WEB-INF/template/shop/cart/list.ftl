	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				// 修改
				$document.on("success.shopec.modifyCart", function(event, data) {
				...
				// 移除
				$document.on("success.shopec.removeCart", function(event, data) {
					var $element = $(event.target);
				...
				// 清空
				$document.on("success.shopec.clearCart", function() {
					location.reload(true);
				});
