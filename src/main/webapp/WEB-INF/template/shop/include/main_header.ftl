	...
	[#escape x as x?js_string]
		...
		$().ready(function() {
			...
			$mainHeaderMemberInfo.on("click", "a.logout", function() {
				$document.trigger("loggedOut.shopec.user", $.getCurrentUser());
			});
			...
			// 购物车数量
			$document.on("complete.shopec.setCurrentCartQuantity", function(event, quantity) {
				...
				<div id="mainHeaderProductSearch" class="product-search clearfix">
					<form action="[#if searchType == "STORE"]${base}/store/search[#else]${base}/product/search[/#if]" method="get">
						<div class="input-group">
							...
							<input name="keyword" class="form-control" type="text"[#if !store??] value="[#if searchType == "STORE"]${storeKeyword}[#else]${productKeyword}[/#if]"[/#if] placeholder="购买、咨询QQ/微信:187048359" autocomplete="off" x-webkit-speech="x-webkit-speech" x-webkit-grammar="builtin:search">
